...
public class MainActivity extends AppCompatActivity {
    ...
    /*
    Java에서의 출발점은 main()메서드였듯이, 안드로이드의 출발점은 onCreate()메서드이다.
    수명주기(Lifecycle) 메서드 중 첫번째로 실행된다.
     */
    @Override
    ...
    View.OnClickListener listener = new View.OnClickListener(){
        ...
        public void onClick(View v) {
            /*
            버튼을 누를때 전달되는 View객체를 통해 눌러진 버튼의 id를 알아낼수있다.
             */
            if(v.getId()==R.id.btnCall){
                //전화걸기 버튼을 눌렀다면 입력상자에 입력된 전화번호를 가져와서 문자열로 변경한다.
    ...
    /*
    리스너 부착3 : 버튼의 onclick속성을 이용한다. 이때 호출할 메서드명을 기술하면
        해당 메서드를 즉시 호출할 수 있다. 단, 메서드 선언시 매개변수로 View객체를
        반드시 사용해야 한다.
     */
    public void myBtnClick(View view){
        Toast.makeText(view.getContext(),
                "onClick속성을 이용한 버튼입니다. 전 짧아요~",
